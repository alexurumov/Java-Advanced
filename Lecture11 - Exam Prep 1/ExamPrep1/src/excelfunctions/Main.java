...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (cmdArgs[0].equals("hide")) {
            hideColumnAndPrint(matrix, cmdArgs[1]);
        } else if (cmdArgs[0].equals("sort")) {
            sortByColumnAndPrint(matrix, cmdArgs[1]);
        } else {
            filterByColumnAndPrint(matrix, cmdArgs[1], cmdArgs[2]);
        }
    ...
    private static void hideColumnAndPrint(String[][] matrix, String hiddenColumn) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                if (j == hiddenColumnIndex) { continue; }
                System.out.print(matrix[i][j]);
                if (j != matrix[i].length - 1) {
                    if (!(j == matrix.length - 2 && hiddenColumnIndex == j + 1)){
                        System.out.print(" | ");
                    }
                }
            }
